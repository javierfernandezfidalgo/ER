...
contains
   ...
   Subroutine filterSolutions(mesh)
      ...
      do eID =1, mesh % no_of_elements
         ! print*, sem_filterA%mesh%elements(eID)%storage%Q(IRHOU:IRHOW,2,2,2)
         ...
         ! print*, sem_filterA%mesh%elements(eID)%storage%Q(IRHOU:IRHOW,2,2,2) ;read(*,*)
      end do
   ...
   Subroutine getER(mesh)
      ...
      do eID =1, mesh % no_of_elements
         ...
         ! call sem_filterA % mesh % storage % elements (eID) % InterpolateSolution (mesh % storage % elements(eID), sem_filterA % mesh % nodeType , .false. )
         ! call sem_filterB % mesh % storage % elements (eID) % InterpolateSolution (mesh % storage % elements(eID), sem_filterB % mesh % nodeType , .false. )

         call elementMomentumIntegral(mesh % elements(eID), integralSolution)
         call elementMomentumIntegral(sem_filterA % mesh % elements (eID), integralFilterA)
         call elementMomentumIntegral(sem_filterB % mesh % elements (eID), integralFilterB)

         ER = sum(POW2(integralSolution - integralFilterA)) / (sum(POW2(integralSolution - integralFilterB)) + 1E-15)
         !  ER = sum(POW2(integralSolution)) / (sum(POW2(integralSolution)) + 1E-10)

         ! print*, 'valores'
         ! print*, integralSolution
         ! print*, integralFilterA
         ! print*, integralFilterB
         ! print*, sum(POW2(integralSolution - integralFilterA))
         ! print*, sum(POW2(integralSolution - integralFilterB))
         ! print*, 'ER = ', ER!;READ(*,*)
         ...
         ! print*, sum(POW2(integralSolution - integralFilterB))
         ! print*, ER;READ(*,*)
         ! if (sum(POW2(integralSolution - integralFilterA))< 1E-6) ER = 0.0_RP
...
!
   ...
   Subroutine elementMomentumIntegral(e,integral)
      use ElementClass
      use NodalStorageClass
      use PhysicsStorage
      Implicit None
      ...
      associate ( wx => NodalStorage(e % Nxyz(1)) % w, &
         ...
         do k = 0, e % Nxyz(3)  ; do j = 0, e % Nxyz(2) ; do i = 0, e % Nxyz(1)
                  !    integral= integral + wx(i) * wy(j) * wz(k) * e % storage % Q(IRHO,i,j,k) * e % geom % jacobian(i,j,k)
                  ...
                  ! PRINT*,  e % storage % Q(IRHOU:IRHOW,i,j,k)
                  ! PRINT*,  e % storage % Q(IRHO,i,j,k)
                  ! PRINT*,  e % storage % Q(IRHOU:IRHOW,i,j,k) /e % storage % Q(IRHO,i,j,k);READ(*,*)
               end do            ; end do           ; end do
!
      end associate
   End Subroutine elementMomentumIntegral
